...
parser.add_argument("--base-model-path", type=str, default="/models/llava-v1.6-vicuna-7b-hf",
...
if args.dataset == "MMT-Bench":
    # TEST SET : /processed_data/test.jsonl (Split in ge_data/mmt_train_test_split.py)
    ...
    ds = load_dataset(
        "parquet", 
        # TEST SET : test-00000-of-00001-f0e719df791966ff.parquet
        data_files={'train': 'datasets/ScienceQA/data/test-00000-of-00001-f0e719df791966ff.parquet', 'validation': 'datasets/ScienceQA/data/validation-00000-of-00001-6c7328ff6c84284c.parquet'},
        split="train"
    )
    ...
    ds = load_dataset(
        ...
        data_files={'train': 'datasets/ChartQA/data/test-00000-of-00001.parquet'},
        split="train"
    )
    ds = ds.filter(lambda record: record["image"] is not None)  
    ds = ds.shuffle(seed=42)
    ...
    ds = load_dataset(
        "parquet", 
        # TEST SET: testmini-00000-of-00001-725687bf7a18d64b.parquet
        data_files={'train': 'datasets/MathVista/data/test-00001-of-00002-6a611c71596db30f.parquet'},
        split="train"
    )
    #ds = ds.filter(lambda record: record["image"] is not None)  
    ds = ds.shuffle(seed=42)
    ...
    ds = load_dataset(
        ...
        data_files={'train': 'datasets/OCRBench-v2/data/test-00000-of-00011.parquet'},
        split="train"
    )
    ds = ds.shuffle(seed=42)
    ds = ds.select(range(sample_num))
